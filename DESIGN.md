...
- **トップ** = 聴く（配信先のボタン）/ この番組 / 出演 / 最新の回（全文）/ これまでの回 / お便り。
  これまでの回は 1 行ずつ、新しい 5 回だけ（`build.py` の `PAST_SHOWN`）。それより多くなったら、その下に
  「すべての回（N 回）」ページ `/ep/` へのリンクが自動で出る（年ごとの見出し、年が 2 つ以上なら上に年への飛び先）。
  最新の回のすぐ下に置いて回の一覧をひとかたまりにし、何回まで増えてもトップの長さとお便りの欄の位置が変わらないようにした。
  2026-09-25 までは古い回をトップの折りたたみに入れていたが、開くと全部が縦に並んでお便りの欄が沈むのでやめた（所有者判断）。
  すべての回のページと回のページの下には「番組へのお便り」（トップのはがき）へのリンクを置く
...
- **手紙を失わない（2026-09-25、所有者判断）**: 回の欄に入れる処理がどんな理由で失敗しても、本文の頭に「［第N回について］」を
  書いて回を決めない形で入れる（Cloudflare の log に 1 行残す）。それまでは想定外の失敗で送った人に「送れませんでした」と出て、
  手紙は消えていた（手元で episode に必ず失敗する表を作り、直す前 = HTTP 500 で消える / 直した後 = 残る、を確かめた）
