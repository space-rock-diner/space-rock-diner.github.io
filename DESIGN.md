...
- **本番での確認（2026-09-25 公開時）**: 全ページと画像が 200、`/#ep1` → `/ep/1/` の転送、390px 幅でのプレーヤー・確認欄・
  はがきの幅。**回つきのお便りの保存（episode 欄を足す処理）は本番ではまだ通っていない**（試しに送るには確認欄を通す必要がある）。
  手元の確認は上の「episode 欄の足し方」
